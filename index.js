...
console.log('task 1', copy(originalFlavors,'originalFlavorsCopy'));
...
function getFlavorByIndex(array,i){
    return(array[i]);
} console.log('task 5', getFlavorByIndex(originalFlavors,"i"));
...
function removeFlavorByName(array,byeFlavor){
     let i;
    for(i=0;i<array.length;i++){
        if(array[i]===byeFlavor){
            array.splice(i,1);
            return(array);
        }
    }
...
function filterByWord(array, string){
    ...
    for(let i=0;i<array.length;i++){
        ...
        if(array[i].includes(string)){
            //push that index to the new string
...
function getAverageWordLength(array){
    ...
    for(i=0;i<array.length;i++){
        var str = array[i];
        wordCount.push(str.length);
    }
    var total = 0;
    for (i=0;i<wordCount.length;i++){
        total+=wordCount[i];
    } var avg = total/wordCount.length;
    return(avg);
    ...
    for (let g =0;g<31;g++){
        ...
        if(randomArray<=0.25){
            ...
            for(let i=0;i<array1.length;i++){
                if(randomIndex<=((1/array1.length)*i)){
                    randomFlavors.push(array1[g]);
                }
            }
            ...
            for(let i=0;i<array2.length;i++){
                if(randomIndex<=((1/array2.length)*i)){
                    randomFlavors.push(array2[i]);
                }
            }

        }
        else if(randomArray<=0.75){
            let randomIndex = Math.random();
            for(let i=0;i<array3.length;i++){
                if(randomIndex<=((1/array3.length)*i)){
                    randomFlavors.push(array3[i]);
                }
            }
            ...
            for(let i=0;i<array4.length;i++){
                if(randomIndex<=((1/array4.length)*i)){
                    randomFlavors.push(array4[i]);
                }
            }
